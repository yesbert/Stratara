...
**One of two Why-Stratara hero samples.** Shows the hash-chain audit trick that Stratara's `EventStreamHashing` worker performs against PostgreSQL — distilled to ~320 lines of in-memory code so the *idea* is visible without a database.
