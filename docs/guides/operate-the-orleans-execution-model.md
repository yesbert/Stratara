...
and it can do that from inside the running cluster, because returning a checkpoint to the beginning is the one
write that is not held to the reader's name: rebuild the read model with `IProjectionRebuilder.RebuildAsync`, or
run a full replay, and the checkpoints are taken over by the host's own reader. The
[reset](#reset-what-the-model-keeps), which needs the deployment stopped, is for the whole model rather than for
one read model.
