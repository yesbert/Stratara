...
| `ResilienceNames.CommandDispatcher` | Up to four attempts, exponential backoff; never retries `CommittedEventsNotPublishedException` | Bounded retry around command dispatch |
| `ResilienceNames.EventBundleDispatcher` | Up to four attempts, exponential backoff; never retries `CommittedEventsNotPublishedException` | Bounded retry around bundle dispatch |
| `ResilienceNames.MessageBus` | Retries indefinitely, behind a circuit breaker; never retries `CommittedEventsNotPublishedException` | Broker traffic, where dropping the message is worse than waiting |
...
| `ResilienceNames.ProjectionReplayBatch` | Five attempts in all, exponential backoff from one second with jitter, retries any exception except cancellation and `CommittedEventsNotPublishedException` | The projection-replay worker runs each batch under it; a read-store timeout or a dropped connection mid-rebuild is retried, and a failure that outlasts every attempt ends the replay |
