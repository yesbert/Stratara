...
| `services.AddNpgsqlReadDbContextFactory<TContext>()` | The same for a read-store context, plus a scoped `IProjectionsUnitOfWork` / `IReadUnitOfWork` over it unless the host registered its own, and the `IForgottenTenantStore` that projections declaring `IForgetsDeletedTenants` need |
...
| `services.AddStrataraForgottenTenants<TReadContext>()` | Keeps, in the read context, the tenants each projection declaring `IForgetsDeletedTenants` has seen deleted. `AddNpgsqlReadDbContextFactory` already calls it; call it for a read context registered another way |
