...
public class SagaWorkerTests
{
    ...
    public async Task PrecedingFactMissing_IsRetriedAndSucceeds_LoggingOneWarning()
    {
        var stream = Guid.NewGuid();
        var calls = 0;
        var logger = new Mock<ILogger<SagaWorker>>();
        ...
        logger.Verify(
            l => l.Log(
                LogLevel.Warning,
                It.Is<EventId>(e => e.Id == Stratara.Diagnostics.LogEvents.Saga.PrecedingFactMissing),
    ...
    {
        public List<Deposited> Dispatched { get; } = [];
    ...
    public async Task A_bundle_with_an_unregistered_event_no_saga_handles_is_dispatched()
    {
        ...
        var saga = new DepositSaga();
        var harness = new Harness((_, _) => Task.CompletedTask,
            realMapper: mapper,
            configure: services =>
            {
                services.AddScoped<ISaga>(_ => saga);
                services.AddScoped<ISagaHandler>(_ => new SagaHandler(new SagaMethodInvoker()));
                services.AddScoped<ISagaManager>(sp => new SagaManager(
                    NullLogger<SagaManager>.Instance, sp.GetRequiredService<ISagaHandler>(), [saga]));
            });
        var stream = Guid.NewGuid();
        ...
        Assert.Equal([new Deposited(5m)], saga.Dispatched);
    }
    ...
    private sealed class ScriptedSagaManager(Func<IReadOnlyList<IEvent>, CancellationToken, Task> behaviour) : ISagaManager
    {
        private int _calls;
    ...
    private sealed class Harness
    {
            ...
            IEventMapperFactory? realMapper = null, Action<IServiceCollection>? configure = null)
        {
            ...
            Sut = new SagaWorker(
                logger ?? NullLogger<SagaWorker>.Instance,
                MessageBus.Object,
                ...
                Options.Create(new SagaOptions { DegreeOfParallelism = degreeOfParallelism }));
        }
