...
public sealed class StoppingSiloTests(PostgreSqlFixture postgres, RedisFixture redis, RabbitMqFixture rabbit)
{
    ...
    public async Task A_recorded_command_whose_handler_waits_on_its_token_observes_the_stop_and_completes_on_the_remaining_silo()
    {
        ...
        // The handler that stopped with its silo counts no attempt, so the record carries none when it is resumed.
        Assert.Equal(0, await RecordedIntentHost.ScalarAsync<int>(
            postgres.ConnectionStringFor("poc_stopping_store"), "SELECT attempt_count FROM outbox_entry WHERE id = @id", ("id", intentId)));
