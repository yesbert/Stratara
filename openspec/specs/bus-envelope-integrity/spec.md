...
- **THEN** it is refused and not dispatched
...
- **WHEN** an unsigned or invalid message arrives and the mode is permissive
- **THEN** it is dispatched anyway, and the failure is recorded so an operator can see which
  publishers are not yet signing
