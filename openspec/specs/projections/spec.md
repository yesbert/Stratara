...
- **WHEN** a replay is active in one host and another host sharing the coordination state dispatches
- **THEN** the other host's dispatch is suppressed as well
...
- **THEN** the other host is unaffected — it never learned of the replay, which is what the start-up
  warning of the first host said would happen
...
coordination store, and in the shared store where it registers one. A host SHALL start and dispatch
in either case; the absence of a shared store SHALL NOT be a start-up or first-dispatch failure.
...
- **THEN** the replay state is held in that store, and a replay requested in one host is seen as
...
- **THEN** it records a warning once, at start-up, saying that replay coordination is per process
  and naming what registers the shared store
