...
- [ ] 1.1 A failing test on today's code: a probe projection depending on a scoped service that captures
...
- [ ] 1.2 `ProjectionGrain.ApplyBatchAsync` applies the batch in session runs, one scope per run, the session
...
- [ ] 1.3 The same for `SagaGrain.ApplyBatchAsync` with the saga manager and the processes, and the same
      ...
      construction*).
- [ ] 1.4 `ISagaProcessGrain.HandleAsync` overload carrying the recorded session in a serialisable carrier;
...
- [ ] 1.5 Documentation: the migration guide says a read is multi-tenant and each entry is applied under
...
- [ ] 2.1 A failing test on today's code: two `ProjectionCheckpointStore` instances over one database, the
...
- [ ] 2.2 `IProjectionCheckpointStore.AdvanceAsync(projection, partition, reader, from, to, ct)` with a
...
- [ ] 2.3 The shipped store: conditional update by position and reader; insert on an absent row at `from == 0`
...
- [ ] 2.4 The loop advances instead of setting, after a batch and after a cut batch, and invalidates its
...
- [ ] 3.1 `StoreReaderSettings` carries the partition count; `StoreReaderGrain.OnActivateAsync` retires a grain
...
- [ ] 3.2 A cluster at four partitions with a reader's reminder for partition 3, reset and restarted at two: the
...
- [ ] 4.1 `ReplaySuspensionTracker` (singleton); the hold, release and retirement events in `LogEvents.Orleans` at the next free ids of the band (`117_114`–`117_116` as of #109, renumbered if taken) and in `OrleansLog`;
      ...
      registrations.
- [ ] 4.2 Three runs under an active replay log one hold, the first run after it one release, and nothing
...
- [ ] 5.1 `docs/reference/log-events-schema.md` rows for the three events;
...
- [ ] 6.1 `./scripts/local-gauntlet.sh` green; `Projections`, `Sagas` and `Singleton` integration namespaces
      green against PostgreSQL. Verify: the run output, recorded here.
- [ ] 6.2 `openspec validate keep-a-store-reader-batch-tenant-correct-and-its-checkpoint-guarded --strict`
