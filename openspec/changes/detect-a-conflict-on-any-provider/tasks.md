...
- [ ] 1.1 `src/Stratara.Abstractions/Abstractions/EventSourcing/IStoreConflictDetector.cs` (new):
...
- [ ] 2.1 `src/Stratara.EventSourcing.EntityFrameworkCore/WriteStore/PostgresConflictDetector.cs`
...
- [ ] 2.2 `src/Stratara.Testing.EntityFrameworkCore/SqliteConflictDetector.cs` (new,
...
- [ ] 3.1 `src/Stratara.Infrastructure/EventSourcing/EventSource.cs`: take
      ...
      returns nothing.
- [ ] 3.2 A test with an empty detector set on the SQLite host (register the context without
...
- [ ] 4.1 `docs/reference/di-extensions-cheatsheet.md`: the write-factory row and the test-store row
...
- [ ] 4.2 Regenerate `llms-full.txt` with the generator the documentation tests use. Verify: the
      documentation tests pass.
- [ ] 4.3 `CHANGELOG.md` `[Unreleased]` → *Added*: `IStoreConflictDetector`, registered by the two
...
- [ ] 5.1 `./scripts/local-gauntlet.sh` green; `openspec validate detect-a-conflict-on-any-provider --strict`
      clean.
