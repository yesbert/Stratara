...
- [ ] 1.1 `src/Stratara.Outbox.RabbitMQ/Projections/InProcessProjectionReplayState.cs` (new,
...
- [ ] 1.2 Tests in `tests/Stratara.Outbox.RabbitMQ.Tests/Projections/InProcessProjectionReplayStateTests.cs`
...
- [ ] 2.1 `src/Stratara.Diagnostics/LogEvents.cs`: `Projection.ProjectionReplayCoordinationInProcess = 104_012`
...
- [ ] 2.2 `src/Stratara.Outbox.RabbitMQ/DependencyInjection/OutboxServiceCollectionExtensions.cs`:
      ...
      confines.
- [ ] 2.3 Tests in `tests/Stratara.Outbox.RabbitMQ.Tests/DependencyInjection/OutboxServiceCollectionExtensionsTests.cs`:
...
- [ ] 2.4 `tests/Stratara.EventSourcing.WorkerDefaults.Tests/WorkerDefaultsCompositesTests.cs`: each
...
- [ ] 2.5 `src/Stratara.Infrastructure/DependencyInjection/CachingServiceCollectionExtensions.cs`:
...
- [ ] 3.1 `docs/guides/write-a-projection.md`, replay section: one host needs no Redis; a replay
      that must suppress publication in other hosts needs the shared store; name the warning id.
- [ ] 3.2 `docs/getting-started/prerequisites.md` and `docs/getting-started/di-composition.md`:
      Redis listed as optional, with the two things it enables.
- [ ] 3.3 `CHANGELOG.md` `[Unreleased]` → *Fixed*: composites no longer require Redis to start; the
      in-process fallback, its confinement and warning `104_012`; Redis-backed hosts unchanged.
- [ ] 3.4 Regenerate `llms-full.txt` if the registration summaries or log-event inventory changed.

## 4. Gate

- [ ] 4.1 `./scripts/local-gauntlet.sh` green; `openspec validate --strict` clean.
