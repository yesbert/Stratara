...
- [ ] 1.1 A two-silo kill test beside the worker-failover one: the keeper activated on the in-process silo
...
- [ ] 2.1 The bookkeeping moves into an internal `PermitLedger` (table, limit, lease, activation time,
      ...
      one call each.
- [ ] 2.2 Inside one lease of activation the ledger refuses an acquisition of a unit it does not hold and
...
- [ ] 2.3 `IHeavyWorkPermitGrain.ReclaimAsync(Guid unitId, SiloAddress holder)` with an `[Alias]`, and
...
- [ ] 3.1 `LogEvents.Orleans.PermitReclaimRefused` at the next free id in the band (117_114 as of #109), and
...
- [ ] 3.2 `PermitRenewal` keeps whether the permit is held: a taken reclaim sets it, a refused one clears it
...
- [ ] 3.3 The kill test of 1.1 passes; the worker-failover test and `HeavyBurstTests`, `HeavyNonYieldingTests`,
...
- [ ] 4.1 `docs/guides/operate-the-orleans-execution-model.md` — *Heavy commands and their aggregate*: the
...
- [ ] 4.2 `docs/reference/log-events-schema.md` gains the row; `docs/concepts/orleans-execution-model.md`
...
- [ ] 5.1 `./scripts/local-gauntlet.sh` green; the `HeavyWork` and `Hosting` integration namespaces green
      against PostgreSQL, Redis and RabbitMQ. Verify: the run output, recorded here.
- [ ] 5.2 `openspec validate keep-the-heavy-work-bound-across-a-permit-owner-failover --strict` passes.
      Verify: the output.
