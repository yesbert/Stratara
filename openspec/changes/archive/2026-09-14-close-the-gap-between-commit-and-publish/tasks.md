...
- [x] 1.1 `src/Stratara.Abstractions/Abstractions/Outbox/IEventBundleOutboxDispatcher.cs`:
...
- [x] 1.2 `src/Stratara.Contracts/Messages/EventBundle.cs` (or wherever the record lives): an
      optional storage id, null on the default path. Verify: existing contract tests unchanged;
      serialisation round-trip test covers both null and set. *Done 2026-09-14 without touching
      `Stratara.Contracts` (design D2, changed during apply): the scoped dispatcher remembers the
      stored id per bundle instance, and `IOutboxRepository` gains a default-implemented
      `AddAsync(Guid id, …)` overload; `OutboxRepository` implements it.*
- [x] 1.3 `src/Stratara.Outbox.RabbitMQ/Outbox/OutboxOptions.cs`: `DurableBundles` (default
      `false`), XML docs stating what it closes and what it costs. Verify: options test shows the
      default.

## 2. The save

- [x] 2.1 `src/Stratara.Infrastructure/EventSourcing/EventSource.cs` `SaveChangesAsync`: when the
...
- [x] 3.1 `EventBundleOutboxDispatcher`: `StoreEventBundleAsync` writes the outbox row under the
      given transaction and remembers its id by bundle instance (weakly — see 1.2);
      `EnqueueEventBundleAsync` on the durable
...
- [x] 3.2 Integration test in `tests/Stratara.Outbox.RabbitMQ.IntegrationTests` with the PostgreSQL
      ...
      assertions in one test class named for the spec scenarios. *Done 2026-09-14 in
      `tests/Stratara.Orleans.IntegrationTests/Outbox/DurableBundlesTests.cs`, because that project
      already has the PostgreSQL and RabbitMQ fixtures the RabbitMQ integration project lacks; "bus
      stopped" is a topic with no bound queue, which mandatory routing reports as a refusal — 2/2.*
- [x] 3.3 The kill test from the PoC, on the bus path with `DurableBundles = true`: the harness in
...
- [x] 4.1 Run the PoC's append benchmark (`tests/Stratara.Orleans.Benchmarks` `--append-throughput`)
...
- [x] 5.1 `docs/guides/outbox-setup-rabbitmq.md` and `outbox-setup-azureservicebus.md`: the window
...
- [x] 5.2 Regenerate `llms-full.txt` with the generator the documentation tests use. Verify: the
      documentation tests pass.
- [x] 5.3 `CHANGELOG.md` `[Unreleased]` → *Added*: `DurableBundles` and what it closes; the interface
      members with their defaults; → *Documentation*: the window is now stated. Verify: the entry
      says the default is unchanged.

## 6. Gate

- [x] 6.1 `./scripts/local-gauntlet.sh` green; the RabbitMQ integration suite green with Docker;
