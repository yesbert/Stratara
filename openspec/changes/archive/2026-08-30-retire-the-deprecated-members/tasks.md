...
- [x] 1.1 Confirm the six `[Obsolete]` sites are still exactly the six named in `proposal.md` —
...
- [x] 1.2 Confirm no framework code calls a type-less snapshot overload. Verify: every call to
...
- [x] 1.3 Confirm `AuthorizationExceptionMiddleware` is registered nowhere but the extension being
...
- [x] 2.1 Delete both obsolete members from
...
- [x] 2.2 Delete their implementations from
...
- [x] 2.3 Check the `<see cref="..."/>` in `src/Stratara.Abstractions/EventSourcing/Snapshot.cs:26`
...
- [x] 2.4 Verify nothing downstream broke: `dotnet test tests/Stratara.Infrastructure.Tests` and
...
- [x] 3.1 Delete `AddNpsqlWriteDbContextFactory<TDbContext>` from
...
- [x] 3.2 Rewrite the rename note in
...
- [x] 3.3 `dotnet test tests/Stratara.EntityFrameworkCore.Tests` green, and the documentation suite
...
- [x] 4.2 Delete `src/Stratara.Infrastructure/Middlewares/AuthorizationExceptionMiddleware.cs`, per
...
- [x] 4.4 Remove the now-impossible double-registration warning from the XML documentation of
      ...
      at a deleted type.
- [x] 4.5 `dotnet test tests/Stratara.Infrastructure.Tests` green, and the problem-details suite green.

## 5. Bring the specification and the derived documentation into line

- [x] 5.1 The `authorization` delta in `specs/authorization/spec.md` replaces the requirement
      *"A denial reaches an HTTP caller as 403"*. Verify: `openspec validate retire-the-deprecated-members --strict`
      ...
      time.
- [x] 5.2 Rewrite `docs/guides/write-a-validator.md:116`. The paragraph introduces
...
- [x] 5.3 Rewrite `docs/guides/enforce-tenant-isolation.md:99`, which names
      ...
      amended requirement.
- [x] 5.4 Rewrite the same sentence in `src/Stratara.Mediator/README.md:75`. Verify: the README ships
      inside the package, so re-read it for the `<see cref>`-free plain-text form the other README
      passages use.
- [x] 5.5 Confirm no derived page still names a removed member. Verify:
...
- [x] 6.1 CHANGELOG entry under `[Unreleased]` as **BREAKING**, with a migration table: each removed
...
- [x] 6.2 Give the 403 removal its own paragraph rather than a table row, per `design.md`: it is the
...
- [x] 6.3 Note the `ISnapshotRepository` implementor case: removing an interface member leaves their
...
- [x] 7.1 `./scripts/local-gauntlet.sh` green, including the documentation tests that compile every
      shipped example.
