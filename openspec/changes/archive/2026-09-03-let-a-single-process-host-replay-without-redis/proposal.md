...
- Additive on the published surface: a patch release. Source: the 2026-09-03 briefing for the first
  consumer built from the published packages (`yesbert/Stratara.Examples`), which found that every
  dispatching composite failed to resolve without a Redis connection no composite registers.
