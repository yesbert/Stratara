...
- [ ] 1.1 In `tests/Stratara.Projections.Tests/Services/ProjectionReplayWorkerTests.cs`, a replay through
...
- [ ] 1.2 In `tests/Stratara.Testing.Orleans.Tests`, a store-reading projection whose partition holds such
  an event. Assert `WaitForReadersAsync` completes. Confirm it stalls on `main`.

## 2. The mapper

- [ ] 2.1 Add `EventRelevance` (`ForTypes`, `AnyResolvable`, `Includes`, `MayName`) and the two
...
- [ ] 2.2 Implement them in `EventMapperFactory`, as `design.md` describes, with the effective name
...
- [ ] 2.3 `tests/Stratara.Shared.Tests`, one test per rule:
...
- [ ] 3.1 `ProjectionWorker` and `ProjectionReplayWorker` pass the union of their projections' relevant
  types. Cover a bundle with an unregistered irrelevant event in `ProjectionWorkerTests`.
- [ ] 3.2 `SagaWorker` passes the union of its sagas' relevant types. Cover the same case in
  `SagaWorkerTests`.
- [ ] 3.3 `ProjectionGrain` passes its projection's relevant types. `SagaReaderGrain` passes a stateless
  ...
  pre-check.
- [ ] 3.4 Tasks 1.1 and 1.2 pass, and every test project that references the changed packages still
  passes.

## 4. Documentation

- [ ] 4.1 `docs/guides/write-a-projection.md`: an event no projection handles is not read, and its type
  need not be registered. Correct the event-only-host section, which currently implies the whole domain
  must be registered.
- [ ] 4.2 `docs/guides/write-a-saga.md`: the same for sagas, and the process reader's warning.
- [ ] 4.3 `docs/reference/di-extensions-cheatsheet.md`: `AddDomainEventTypesFromAssemblyContaining` is no
  longer needed to keep irrelevant events from failing a host.
- [ ] 4.4 `CHANGELOG.md` under Unreleased: Fixed, Added (`EventRelevance` and the overloads), and an
  Upgrading note on renamed handled types.

## 5. Gate

- [ ] 5.1 `openspec validate leave-an-irrelevant-event-unread --strict`
