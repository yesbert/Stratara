...
| `Messaging/` | `RabbitMqBus` — `IMessageBus` over RabbitMQ. A worker subscription is a quorum queue `<subscription>.v2` with `<subscription>.dead-letter` beside it; a message a handler cannot take is redelivered under `MessageRetryOptions` and then dead-lettered. A subscription holds at most `Messaging:PrefetchCount` messages (default 16), and the host waits for a stopping subscription's running handler to settle before it counts as stopped. Azure Service Bus ships as the sibling `Stratara.Outbox.AzureServiceBus` package. |
