...
/// 96-bit nonces and 128-bit authentication tags — the parameters fixed in <see cref="SecurityConstants"/>.
/// </summary>
/// <remarks>
/// <para>
...
/// ciphertexts to a tenant scope (<c>tenantId||scope</c>) so a ciphertext copied from one tenant
/// to another fails decryption.
/// </para>
/// <para>
/// <strong>Nonce uniqueness:</strong> a nonce MUST never repeat under the same key. This factory
/// fills the nonce from <see cref="RandomNumberGenerator"/>, which gives a sufficiently low
/// collision probability for the typical Stratara workload. Callers are responsible for rotating
/// the key before the per-key encryption count approaches the GCM birthday bound (~2^32).
/// </para>
/// </remarks>
internal sealed class AesGcmEncryptionFactory : IEncryptionFactory
{
    ...
    {
        var nonce = GC.AllocateUninitializedArray<byte>(SecurityConstants.NonceSize);
        var tag = GC.AllocateUninitializedArray<byte>(SecurityConstants.TagSize);
        var ciphertext = GC.AllocateUninitializedArray<byte>(plaintext.Length);

        RandomNumberGenerator.Fill(nonce);

        using var gcm = new AesGcm(key, SecurityConstants.TagSize);
