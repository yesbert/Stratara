...
# Optional: SONARQUBE_PROJECT_KEY (default: stratara)
#
...
if [[ -z "${SONARQUBE_URL:-}" ]]; then
  echo "::warning::SONARQUBE_URL not set, skipping result export"
  exit 0
fi

if [[ -z "${SONARQUBE_TOKEN:-}" ]]; then
  echo "::warning::SONARQUBE_TOKEN not set, skipping result export"
  exit 0
fi

if [[ -z "${BUILD_ARTIFACT_DIR:-}" ]]; then
  echo "::error::BUILD_ARTIFACT_DIR not set"
  exit 1
fi

PROJECT_KEY="${SONARQUBE_PROJECT_KEY:-stratara}"
...
sonar_curl() {
  local url="$1"
  local http_code
  http_code=$(curl -s -o /tmp/sonar_response.json -w "%{http_code}" -H "$AUTH_HEADER" "$url")
...
if ! curl -sf "${SONARQUBE_URL}/api/system/status" > /dev/null 2>&1; then
  echo "::warning::Cannot reach SonarQube API, skipping result export"
  exit 0
fi

echo "Verifying authentication..."
AUTH_RESPONSE=$(sonar_curl "${SONARQUBE_URL}/api/authentication/validate" 2>&1) || {
  echo "::warning::SonarQube authentication endpoint unreachable, skipping result export"
  exit 0
}
if ! echo "$AUTH_RESPONSE" | jq -e '.valid == true' > /dev/null 2>&1; then
  echo "::warning::SonarQube token is invalid (authentication returned valid=false), skipping result export"
  exit 0
fi
...
while true; do
  RESPONSE=$(sonar_curl \
    ...
    echo "::warning::Failed to fetch issues (page $PAGE)"
    break
  }
  TOTAL=$(echo "$RESPONSE" | jq '.paging.total // 0')
  ISSUES=$(echo "$RESPONSE" | jq '.issues // []')
  jq -s '.[0] + .[1]' /tmp/all_issues.json <(echo "$ISSUES") > /tmp/merged.json
  mv /tmp/merged.json /tmp/all_issues.json
...
echo "[]" > /tmp/all_duplicated_files.json
DUP_PAGE=1
while true; do
  DUP_RESPONSE=$(sonar_curl \
    ...
    echo "::warning::Failed to fetch duplicated files (page $DUP_PAGE)"
    break
  }
  ...
  jq -s '.[0] + .[1]' /tmp/all_duplicated_files.json <(echo "$DUP_COMPONENTS") > /tmp/dup_merged.json
  mv /tmp/dup_merged.json /tmp/all_duplicated_files.json
...
jq '.' /tmp/all_issues.json > "$OUTPUT_DIR/issues.json"
...
jq '.' /tmp/all_duplicated_files.json > "$OUTPUT_DIR/duplications.json"

# --- Compute statistics ---

BLOCKERS=$(jq '[.[] | select(.severity == "BLOCKER")] | length' /tmp/all_issues.json)
CRITICALS=$(jq '[.[] | select(.severity == "CRITICAL")] | length' /tmp/all_issues.json)
MAJORS=$(jq '[.[] | select(.severity == "MAJOR")] | length' /tmp/all_issues.json)
MINORS=$(jq '[.[] | select(.severity == "MINOR")] | length' /tmp/all_issues.json)
INFOS=$(jq '[.[] | select(.severity == "INFO")] | length' /tmp/all_issues.json)
...
DUP_FILE_COUNT=$(jq 'length' /tmp/all_duplicated_files.json)
...

{
  ...
  if [[ $DUP_FILE_COUNT -gt 0 ]]; then
    ...
    jq -r '.[0:50][] |
      ...
      /tmp/all_duplicated_files.json || true
  ...
  for SEVERITY in BLOCKER CRITICAL MAJOR MINOR INFO; do
    COUNT=$(jq "[.[] | select(.severity == \"$SEVERITY\")] | length" /tmp/all_issues.json)
    if [[ $COUNT -gt 0 ]]; then
      echo "---"
      echo ""
